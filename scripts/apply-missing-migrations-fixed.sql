-- ============================================================================
-- Apply Missing Migrations Script (FIXED VERSION)
-- ============================================================================
-- Purpose: Apply the 3 migrations that were skipped due to naming
-- Run this in Supabase Dashboard SQL Editor
-- Project: evrcwtsexlamacawofxo
-- Fixed: Removed IF NOT EXISTS from ALTER TABLE ADD CONSTRAINT (not supported)
-- ============================================================================
...
-- ============================================================================
-- MIGRATION 1: Cookie Constraints (OPTIONAL - Skip if cookies table doesn't exist)
-- ============================================================================
...
BEGIN
    ...
    -- Only apply if cookies table exists
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'cookies') THEN
        -- Drop constraint if exists, then add it
        BEGIN
            ALTER TABLE cookies DROP CONSTRAINT IF EXISTS chk_cookie_amount_positive;
            ALTER TABLE cookies ADD CONSTRAINT chk_cookie_amount_positive CHECK (amount > 0);
            RAISE NOTICE '✅ Cookie constraints applied';
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE '⚠️ Cookie constraints skipped (table may not exist)';
        END;
    ELSE
        RAISE NOTICE '⚠️ Cookies table does not exist, skipping...';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'cookie_transactions') THEN
        BEGIN
            ALTER TABLE cookie_transactions DROP CONSTRAINT IF EXISTS chk_cookie_transaction_amount;
            ALTER TABLE cookie_transactions ADD CONSTRAINT chk_cookie_transaction_amount CHECK (amount != 0);
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE '⚠️ Cookie transaction constraints skipped';
        END;
    END IF;
END $$;
...
DO $$
BEGIN
    ALTER TABLE gem_transactions DROP CONSTRAINT IF EXISTS chk_gem_amount_not_zero;
    ALTER TABLE gem_transactions ADD CONSTRAINT chk_gem_amount_not_zero CHECK (amount != 0);
    RAISE NOTICE '✅ Gem amount constraint added';
EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'Failed to add gem amount constraint: %', SQLERRM;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE gem_transactions ADD COLUMN idempotency_key VARCHAR(255) UNIQUE;
        RAISE NOTICE '✅ Idempotency key column added';
    ELSE
        RAISE NOTICE '✅ Idempotency key column already exists';
    END IF;
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '✅ Gem transaction functions created';
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '✅ CometChat webhook URL configured';
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '✅ CometChat sync function created';
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '✅ CometChat trigger created';
END $$;
...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1 FROM pg_tables WHERE tablename = 'gem_transaction_audit_log'
    ) INTO v_audit_exists;
    ...
    SELECT EXISTS (
        SELECT 1 FROM pg_tables WHERE tablename = 'system_settings'
    ...
    SELECT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'profiles_cometchat_sync'
    ...
    RAISE NOTICE '   npx supabase functions deploy cometchat-user-sync';
    RAISE NOTICE '';
    RAISE NOTICE '2. Set CometChat secrets:';
    RAISE NOTICE '   npx supabase secrets set COMETCHAT_APP_ID=167456197b8d940a5';
    RAISE NOTICE '   npx supabase secrets set COMETCHAT_API_KEY=...';
    RAISE NOTICE '   npx supabase secrets set COMETCHAT_REGION=us';
    RAISE NOTICE '';
    RAISE NOTICE '3. Test user sync - create a test profile';
    RAISE NOTICE '';
